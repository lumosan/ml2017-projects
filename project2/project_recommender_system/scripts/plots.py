...
def plot_raw_data(ratings):
    ...
    ax1.plot(np.cumsum(sorted_num_movies_per_user), c='blue')
    ax1.plot(np.full(sorted_num_movies_per_user.shape, 588476), c='red', ls='--')
    ax1.set_xlabel("users")
    ax1.set_ylabel("accumulated number of ratings (sorted)")
    ax1.set_xticks(np.arange(0, ratings.shape[1], int(ratings.shape[1]/5)))
    ax1.grid()

    ax2 = fig.add_subplot(1, 2, 2)
    ax2.plot(sorted_num_movies_per_user, c='blue')
    ax2.set_xlabel("users")
    ax2.set_ylabel("number of ratings (sorted)")
    ax2.set_xticks(np.arange(0, ratings.shape[1], int(ratings.shape[1]/5)))
    ax2.grid()
